...
{
  Selenite Lua Extension Library
...
procedure push_selenite_class(L: plua_State; name: string;
...
type
  TStringFields = (s_after, s_before, s_beginswith, s_between, s_decrease,
    ...
    s_loop, s_match, s_occur, s_random, s_replace, s_replacefirst,
...
begin
  ...
  case TStringFields(GetEnumValue(TypeInfo(TStringFields),
    ...
    s_list:
      push_selenite_class(L, 'sel_stringlist', RegisterSeleniteStrList);
    s_loop:
      push_selenite_class(L, 'sel_listparser', RegisterSeleniteStrListParser);
    s_match:
...
begin
  ...
  case TJSONFields(GetEnumValue(TypeInfo(TJSONFields),
    ...
    j_object:
      push_selenite_class(L, 'sel_json', RegisterSeleniteJSON);
  else
    result := 0;
  end;
end;
...
begin
  ...
  case THTMLFields(GetEnumValue(TypeInfo(THTMLFields),
    ...
    h_parser:
      push_selenite_class(L, 'sel_htmlparser', RegisterSeleniteHTMLParser);
    h_striptags:
...
type
  TDirFields = (d_create, d_delete, d_getdirlist, d_getfilelist, d_packtotar,
    d_unpackfromtar);
...
type
  TUtilFields = (ut_delay, ut_hassoftware);
